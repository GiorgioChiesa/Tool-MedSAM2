...
def create_overlay_video(
    ...
    """Create a video with mask overlays on frames."""
    
    video_dir = os.path.join(base_video_dir, video_name)
    mask_dir = os.path.join(output_mask_dir, "masks", video_name)
    ...
    if not os.path.exists(first_frame_path):
        first_frame_path = os.path.join(video_dir, f"{frame_names[0]}.png")
    
    first_frame = cv2.imread(first_frame_path)
    height, width = first_frame.shape[:2]
    
    # Convert DAVIS palette to BGR colors
    davis_colors = []
    for i in range(0, len(DAVIS_PALETTE), 3):
        r, g, b = DAVIS_PALETTE[i], DAVIS_PALETTE[i+1], DAVIS_PALETTE[i+2]
        davis_colors.append((b, g, r))  # OpenCV uses BGR
    ...
    for frame_name in frame_names:
        ...
        mask_path = os.path.join(mask_dir, f"{frame_name}.png")
        
        if os.path.exists(mask_path):
            mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
            mask_colored = np.zeros((height, width, 3), dtype=np.uint8)
            
            # Apply colors from palette based on mask values
            unique_ids = np.unique(mask)
            for obj_id in unique_ids:
                if obj_id == 0:  # Skip background
                    continue
                obj_mask = mask == obj_id
                color = davis_colors[obj_id % len(davis_colors)]
                mask_colored[obj_mask] = color
...
@torch.inference_mode()
@torch.autocast(device_type="cuda", dtype=torch.bfloat16)
def calculate_performance(args , video_names, score_func=iou_score):

    # Calculate IoU over all videos
    total_iou = 0.0
    total_frames = 0
    
    for n_video, video_name in enumerate(video_names):
        real_path = os.path.join(args.input_mask_dir, video_name)
        output_path = os.path.join(args.output_mask_dir,"masks", video_name)
        ...
        for frame_name in frame_names:
            real_mask_path = os.path.join(real_path, f"{frame_name}.png")
            output_mask_path = os.path.join(output_path, f"{frame_name}.png")
            
            if os.path.exists(real_mask_path) and os.path.exists(output_mask_path):
                real_mask = np.array(Image.open(real_mask_path)) > 0
                output_mask = np.array(Image.open(output_mask_path)) > 0
                
                iou = score_func(real_mask, output_mask)
                video_iou += iou
                video_frames += 1
        
        if video_frames > 0:
            avg_video_iou = video_iou / video_frames
            print(f"Video {video_name}: IoU = {avg_video_iou:.4f}")
            total_iou += video_iou
            total_frames += video_frames
    
    return total_iou / total_frames if total_frames > 0 else 0.0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="directory containing videos (as JPEG, png files) to run inference on",
    )
    parser.add_argument(
        "-m",
        "--input_mask_dir",
    ...
    for n_video, video_name in enumerate(video_names):
        ...
        if not args.track_object_appearing_later_in_video:
            vos_inference(
                ...
                output_mask_dir=os.path.join(args.output_mask_dir, "masks/"),
                video_name=video_name,
        ...
        create_overlay_video(
            ...
            video_name=video_name,
            output_video_dir=args.output_mask_dir,
            alpha=0.6,
        )
    # calculate overall IoU score over all videos
    print("\nCalculating overall IoU score over all videos...")
    overall_iou = calculate_performance(args , video_names, score_func=iou_score)

    print(
        f"completed inference on {len(video_names)} videos -- "
        f"output masks saved to {args.output_mask_dir}\n"
        f"Overall IoU score: {overall_iou:.4f}"
    )
